...
#' 
#' This pipeline processes PacBio HiFi amplicon sequencing data through DADA2
...
if (!is.null(opt$minLen) || !is.null(opt$maxLen) || !is.null(opt$maxEE) || 
    !is.null(opt#!/usr/bin/env Rscript
# PacBio HiFi DADA2 Pipeline with Read ID Tracking
...
run_pacbio_dada2_pipeline <- function(
  ...
  # Step 9: Assign taxonomy (optional)
  message("Step 8: Assigning taxonomy (if reference provided)...")
  # Uncomment and modify path as needed:
  # tax <- assignTaxonomy(seqtab_nochim, "path/to/silva_or_gtdb_ref.fa.gz", multithread = threads)
...
fwd-primer`) || !is.null(opt#!/usr/bin/env Rscript
# PacBio HiFi DADA2 Pipeline with Read ID Tracking
# Supports multiple amplicon types: V3V4, FL-16S, Titan, full operons
...
rev-primer`)) {
  ...
  if (!is.null(opt#!/usr/bin/env Rscript
# PacBio HiFi DADA2 Pipeline with Read ID Tracking
# Supports multiple amplicon types: V3V4, FL-16S, Titan, full operons
...
fwd-primer`)) config$primers$forward <- opt#!/usr/bin/env Rscript
# PacBio HiFi DADA2 Pipeline with Read ID Tracking
# Supports multiple amplicon types: V3V4, FL-16S, Titan, full operons
...
rev-primer`)) config$primers$reverse <- opt#!/usr/bin/env Rscript
# PacBio HiFi DADA2 Pipeline with Read ID Tracking
# Supports multiple amplicon types: V3V4, FL-16S, Titan, full operons
...
cat("Read tracking:", opt#!/usr/bin/env Rscript
# PacBio HiFi DADA2 Pipeline with Read ID Tracking
# Supports multiple amplicon types: V3V4, FL-16S, Titan, full operons
...
tryCatch({
  results <- run_pacbio_dada2_pipeline(
    ...
    threads = opt$threads,
    pool = opt$pool,
    track_reads = opt#!/usr/bin/env Rscript
# PacBio HiFi DADA2 Pipeline with Read ID Tracking
# Supports multiple amplicon types: V3V4, FL-16S, Titan, full operons
...
track-reads`,
    skip_primers = opt#!/usr/bin/env Rscript
# PacBio HiFi DADA2 Pipeline with Read ID Tracking
# Supports multiple amplicon types: V3V4, FL-16S, Titan, full operons
